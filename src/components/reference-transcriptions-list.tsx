...
export function ReferenceTranscriptionsList({ items }: ReferenceTranscriptionsListProps) {
  ...
  const removeMutation = useMutation({
    ...
    mutationFn: async (transcriptionId: string) => {
      ...
      return runRemove({ data: { projectId, transcriptionId } });
    ...
    <Card className="glass p-6 space-y-4 border border-primary/20">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          ...
          {items.length} texto(s) salvos — as 5 mais recentes entram no gerador, VSL e refinar.

          Trechos com análise IA mostram hook e estrutura decompostos.
          Referências orientam estrutura; a promessa vem da URL do gerador.
