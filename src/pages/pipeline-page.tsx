...
export const pipelineSearchSchema = z.object({
  ...
  formato: z.enum(["criativo_curto", "vsl_curta", "all"]).optional(),
});
...
export function PipelinePage({
  ...
  mode: ProductMode;
  routeSearch: PipelineSearch;
}) {
  const config = getProductConfig(mode);
  const defaultFormatoFilter =
    mode === "criativo" ? (routeSearch.formato ?? "criativo_curto") : config.forcedFormato;

  const { status: urlStatus, criativoId: urlCriativoId, export: urlExport } = routeSearch;
  ...
  useEffect(() => {
    if (mode === "criativo" && routeSearch.formato) {
      setFormatoFilter(routeSearch.formato);
    }
  }, [mode, routeSearch.formato]);

  const [formatoFilter, setFormatoFilter] = useState(defaultFormatoFilter);

  const listFormatoSaida =
    mode === "vsl"
      ? ("vsl_curta" as const)
      : formatoFilter === "all"
        ? undefined
        : (formatoFilter as "criativo_curto" | "vsl_curta");

  const { data: rows = [], isLoading, error } = useQuery({
    queryKey: ["criativos", projectId, listFormatoSaida],
    queryFn: () =>
      fetchCriativos({
        data: {
          projectId: projectId!,
          ...(listFormatoSaida ? { formatoSaida: listFormatoSaida } : {}),
        },
      }),
    enabled: !!projectId,
  });
  ...
  const { data: dashStats } = useQuery({
    queryKey: ["dashboard-stats", projectId, mode],
    ...
    <div className="container mx-auto px-4 md:px-6 py-8 max-w-7xl space-y-6">
      ...
      <div className="flex items-end justify-between">
        <div className="space-y-3">
          {mode === "criativo" && (
            <div className="flex gap-1 p-1 rounded-lg bg-muted/50 w-fit">
              {(
                [
                  { id: "criativo_curto", label: "Criativo curto" },
                  { id: "vsl_curta", label: "VSL" },
                  { id: "all", label: "Todos" },
                ] as const
              ).map((tab) => (
                <Button
                  key={tab.id}
                  size="sm"
                  variant={formatoFilter === tab.id ? "default" : "ghost"}
                  className={formatoFilter === tab.id ? "bg-gradient-primary border-0" : ""}
                  onClick={() => {
                    setFormatoFilter(tab.id);
                    navigate({
                      to: "/app/historico",
                      search: {
                        ...(routeSearch.status ? { status: routeSearch.status } : {}),
                        ...(routeSearch.export ? { export: routeSearch.export } : {}),
                        formato: tab.id,
                      },
                    });
                  }}
          ...
          <h1 className="text-3xl font-display font-bold">
            {mode === "vsl" ? "Pipeline VSL" : "Pipeline"}
          </h1>
          <p className="text-muted-foreground mt-1">
            {filtered.length} {mode === "vsl" ? "VSLs" : "criativos"} · pipeline Gerado → Subiu → Rodando → Performando
          </p>
          {urlExport === "pendente" && (
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <Link to={config.geradorPath}>
            <Button size="sm" className="bg-gradient-primary border-0">
              {mode === "vsl" ? "Gerar nova VSL" : "Gerar novos ângulos"}
            </Button>
          </Link>
          <Button
            variant="outline"
              ...
              onValueChange={(v) => {
                ...
                navigate({
                  ...
                  search: {
                    ...(routeSearch.status ? { status: routeSearch.status } : {}),
                    ...(routeSearch.export ? { export: routeSearch.export } : {}),
                    formato: v as "criativo_curto" | "vsl_curta" | "all",
                  },
                });
              }}
            >
      ...
      <Card className="glass overflow-hidden">
        {wsLoading || isLoading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">
            ...
            <Link to={config.geradorPath} className="text-primary-glow underline">Gere ângulos</Link> para começar.
          </div>
        ) : (
          <>
