...
import { getProductConfig, type ProductMode, pipelineSearchForMode } from "@/lib/product-mode";
...
import type { TomCalibracao, NivelConscienciaModo, NivelConscienciaAlvo } from "@/lib/types/enums";
...
import { saveDraftQueue, syncDraftQueueFromServer } from "@/lib/draft-queue";
import { persistBatchChecklist } from "@/components/batch-draft-checklist-host";
import { saveProjectDraftQueue, loadProjectDraftQueue } from "@/lib/criativos.functions";
...
export const geradorSearchSchema = z.object({
...
export type GeradorSearch = z.infer<typeof geradorSearchSchema>;
...
export function GeradorPage({
  ...
  mode: ProductMode;
  search: GeradorSearch;
}) {
  const navigate = useNavigate();
  const config = getProductConfig(mode);
  const { step: urlStep, formato: urlFormato, url: urlSearch, ttfe: ttfeFlag } = search;
  const effectiveFormato = mode === "vsl" ? config.forcedFormato : urlFormato;
  const { organizationId, projectId, currentProject, setWorkspace, currentOrg } = useWorkspace();
  ...
  const [nivelConscienciaModo, setNivelConscienciaModo] = useState<NivelConscienciaModo>("ia");
  const [nivelConscienciaNivel, setNivelConscienciaNivel] = useState<NivelConscienciaAlvo>(4);
  const [nivelConscienciaMin, setNivelConscienciaMin] = useState<NivelConscienciaAlvo>(3);
  const [nivelConscienciaMax, setNivelConscienciaMax] = useState<NivelConscienciaAlvo>(5);
  ...
  const persistDraftQueue = useServerFn(saveProjectDraftQueue);
  const loadQueueServer = useServerFn(loadProjectDraftQueue);

  async function persistQueueToServer(ids: string[]) {
    saveDraftQueue(ids);
    if (projectId && ids.length > 0) {
      try {
        await persistDraftQueue({ data: { projectId, criativoIds: ids } });
      } catch {
        /* fallback local */
      }
    }
  ...
  useEffect(() => {
    if (!projectId) return;
    void loadQueueServer({ data: { projectId } })
      .then((res) => syncDraftQueueFromServer(res.criativoIds))
      .catch(() => undefined);
  }, [projectId, loadQueueServer]);
  ...
  useEffect(() => {
    if (ttfeFlag === "1") {
      ...
      navigate({ to: config.geradorPath, search: { url: urlSearch }, replace: true });
    }
  ...
  useEffect(() => {
    if (mode === "criativo" && urlFormato === "vsl_curta") return;
    if (effectiveFormato && !geracaoId && !resultado && !loadingGeracao) {
      setPendingFormato(effectiveFormato);
      if (urlStep !== "wizard") {
        navigate({ to: config.geradorPath, search: { formato: effectiveFormato }, replace: true });
      }
      toast.info(
        effectiveFormato === "vsl_curta"
  ...
  }, [effectiveFormato, urlFormato, urlStep, geracaoId, resultado, loadingGeracao, navigate, mode, config.geradorPath]);

  useEffect(() => {
    if (effectiveFormato && urlStep === "wizard" && geracaoId && resultado) {
      setEtapa("wizard");
      setWizardStep("producao");
    }
  }, [effectiveFormato, urlStep, geracaoId, resultado]);
  ...
  useEffect(() => {
    ...
    navigate({ to: config.geradorPath, search: { step: "wizard" }, replace: true });
  }, [urlStep, navigate]);
    ...
    try {
      const data = await run({
        data: {
          ...
          tomCalibracao,
          nivelConscienciaModo,
          ...(nivelConscienciaModo === "nivel" ? { nivelConscienciaNivel } : {}),
          ...(nivelConscienciaModo === "faixa"
            ? { nivelConscienciaMin, nivelConscienciaMax }
            : {}),
          projectId: projectId ?? undefined,
          organizationId: organizationId ?? undefined,
        },
  ...
  async function autoCreateSingleDraft(
    ...
    const fmtMap = initFormatoPorAngulo([idx], config.forcedFormato);
    ...
    try {
      ...
      await persistQueueToServer([criativoId]);
      localStorage.removeItem(WIZARD_STORAGE_KEY);
      queryClient.invalidateQueries({ queryKey: ["criativos"] });
  ...
  async function executeCreateDrafts(fmtMapOverride?: Record<number, FormatoOverride>) {
    ...
    try {
      ...
      const draftList = created.map((c) => ({
        ...
        formatoSaida: fmtMap[c.idx]?.formatoSaida ?? config.forcedFormato,
      }));
      ...
      await persistQueueToServer(queueIds);
      ...
      if (created.length > 1) {
        persistBatchChecklist(draftList);
      }
  ...
  return (
    <div className={mode === "vsl" ? "space-y-8 relative -mt-2" : "container mx-auto px-6 py-8 max-w-6xl space-y-8 relative"}>
      {mode !== "vsl" && (
      <AppBreadcrumbs
        items={[
      ...
      <div>
        <h1 className="text-3xl font-display font-bold">
          {mode === "vsl" ? "Gerador de VSL" : "Gerador de ângulos"}
        </h1>
        <p className="text-muted-foreground mt-1">
          {mode === "vsl"
            ? "Briefing → ângulos → roteiro VSL de 2 min em 6 blocos."
            : modoRapido
      ...
      <Card className="glass bg-gradient-card p-6">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label>Nível de consciência (Schwartz)</Label>
            <Select
              value={nivelConscienciaModo}
              onValueChange={(v) => setNivelConscienciaModo(v as NivelConscienciaModo)}
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ia">IA decide (baseado no objetivo)</SelectItem>
                <SelectItem value="nivel">Nível fixo prioritário</SelectItem>
                <SelectItem value="faixa">Faixa min–max</SelectItem>
                <SelectItem value="funil">Pacote funil (1 por nível)</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground">
              Controla a profundidade do argumento: inconsciente (1) até mais consciente (5).
            </p>
          </div>
          {nivelConscienciaModo === "nivel" && (
            <div className="space-y-1.5">
              <Label>Nível prioritário</Label>
              <Select
                value={String(nivelConscienciaNivel)}
                onValueChange={(v) => setNivelConscienciaNivel(Number(v) as NivelConscienciaAlvo)}
              >
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {([1, 2, 3, 4, 5] as const).map((n) => (
                    <SelectItem key={n} value={String(n)}>
                      {NIVEL_CONSCIENCIA_LABELS[n]}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          {nivelConscienciaModo === "faixa" && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>Mínimo</Label>
                <Select
                  value={String(nivelConscienciaMin)}
                  onValueChange={(v) => setNivelConscienciaMin(Number(v) as NivelConscienciaAlvo)}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {([1, 2, 3, 4, 5] as const).map((n) => (
              ...
              <div className="space-y-1.5">
                <Label>Máximo</Label>
                <Select
                  value={String(nivelConscienciaMax)}
                  onValueChange={(v) => setNivelConscienciaMax(Number(v) as NivelConscienciaAlvo)}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {([1, 2, 3, 4, 5] as const).map((n) => (
      ...
      {resultado && etapa === "resultado" && (
        <>
          <div className="flex items-center justify-between">
            ...
            <Link to={config.pipelinePath} search={pipelineSearchForMode(mode)}>
          ...
          <Card className="glass bg-gradient-card p-6">
            ...
            {HttpUrlSchema.safeParse(url.trim()).success && (
              <p className="text-xs text-muted-foreground mt-4 p-3 rounded-lg border border-border/40 bg-background/30">
                ...
                <Link to={config.pipelinePath} search={pipelineSearchForMode(mode)} className="text-primary-glow underline">
                  abra o editor
                </Link>{" "}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setSelectedAngulos(new Set(picks));
                    const map = buildAbTestFormatoMap(resultado.angulos, picks, formatCtx ?? null, mode);
                    const globalFmt = pendingFormato ?? effectiveFormato ?? config.forcedFormato;
                    if (globalFmt) {
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setEtapa("wizard");
                    setWizardStep("producao");
                    navigate({ to: config.geradorPath, search: { step: "wizard" } });
                    ...
                    <AccordionContent className="space-y-4 pt-3 px-5 pb-4">
                      ...
                      {rec && (
                        <div className="p-4 rounded-lg bg-accent/10 border border-accent/30 space-y-2">
                          ...
                          {mode === "vsl" && rec.formato_saida === "vsl_curta" && (
                ...
                {[...selectedAngulos].sort((a, b) => a - b).map((idx) => {
                  ...
                  return (
                    <div key={idx} className="p-4 rounded-lg border border-border/50 bg-background/30 space-y-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          {mode === "criativo" ? (
                            <p className="text-sm text-muted-foreground h-9 flex items-center">
                              {formatoSaidaLabel("criativo_curto")}
                            </p>
                          ) : (
                          <Select
                      ...
                      {mode === "vsl" && fmt.formatoSaida === "vsl_curta" && (
                        <p className="text-xs text-muted-foreground">
      ...
      <Dialog open={!!partialDrafts} onOpenChange={(open) => !open && setPartialDrafts(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-2 max-h-60 overflow-auto py-2">
            {partialDrafts?.map((d) => (
              <Link key={d.id} to={config.editorPath} search={{ criativoId: d.id }} onClick={() => setPartialDrafts(null)}>
