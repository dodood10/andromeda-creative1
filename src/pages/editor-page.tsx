...
import { getCriativo, getLatestCriativo, updateCriativoRoteiro, updateCriativoStatus, getGeradorEtaEstimates, advanceProjectDraftQueue, loadProjectDraftQueue } from "@/lib/criativos.functions";
import {
  avaliarCriativo,
  solicitarExport,
  gerarAudio,
...
import { EditorOnboardingStrip } from "@/components/editor-onboarding-strip";
import { getPlanUsage } from "@/lib/plan.functions";
import { trackMetaInitiateCheckout } from "@/lib/meta-pixel";
import { advanceDraftQueue, draftQueuePosition, syncDraftQueueFromServer, saveDraftQueue } from "@/lib/draft-queue";
import { removeFromBatchChecklist } from "@/components/batch-draft-checklist-host";
...
export function EditorPage({
  ...
  const { criativoId: searchId, focus } = search;
  ...
  const { data: latest, isLoading: loadingLatest } = useQuery({
    queryKey: ["latest-criativo", projectId, mode],
    queryFn: () =>
      fetchLatest({
        data: {
          projectId: projectId!,
    ...
    if (!searchId && !loadingLatest) {
      if (latest?.criativoId) {
        navigate({
          ...
          search: { criativoId: latest.criativoId },
          replace: true,
        });
      }
    }
  }, [searchId, latest, loadingLatest, navigate, config.editorPath]);
      ...
      return (
        <div className="container py-12 text-center">
          <p className="text-muted-foreground">
            {mode === "vsl" ? "Nenhuma VSL ainda." : "Nenhum criativo ainda."}
          </p>
          <Link to={config.geradorPath} className="text-primary-glow underline mt-4 inline-block">
            {mode === "vsl" ? "Gerar VSL" : "Gerar ângulos"}
          </Link>
        </div>
      );
    }
  ...
  return <Editor criativoId={searchId} focus={focus} mode={mode} />;
}

type EditorProps = { criativoId: string; focus?: "audio" | "score" | "media"; mode: ProductMode };

function Editor({ criativoId, focus, mode }: EditorProps) {
  const config = getProductConfig(mode);
  const { user } = useAuth();
  const { projectId, organizationId, currentOrg } = useWorkspace();
  const navigate = useNavigate();
  ...
  const advanceQueueServer = useServerFn(advanceProjectDraftQueue);
  ...
  const [exportCongruenceOpen, setExportCongruenceOpen] = useState(false);
  ...
  useEffect(() => {
    if (!criativo) return;
    if (mode === "criativo" && criativo.formato_saida === "vsl_curta") {
      navigate({
        to: "/app/vsl/editor",
        search: { criativoId, focus },
        replace: true,
      });
    } else if (
      mode === "vsl" &&
  ...
  }, [criativo, mode, criativoId, focus, navigate]);
      ...
      if (kickoff.status === "renderizando") {
        ...
        while (Date.now() < deadline) {
          ...
          if (exportRes.status === "pronto") {
            ...
            queryClient.invalidateQueries({ queryKey: ["criativo", criativoId] });
            removeFromBatchChecklist(criativoId);
            return;
          }
  ...
  async function handleExport() {
    ...
    if (landingAlignment && landingAlignment.score < 70) {
      setExportCongruenceOpen(true);
      return;
    }
    await runExportFlow();
  }
  ...
  async function handleMarcarSubiu() {
    ...
    try {
      ...
      let nextId = advanceDraftQueue(criativoId);
      if (projectId) {
        try {
          const res = await advanceQueueServer({
            data: { projectId, currentCriativoId: criativoId },
          });
          saveDraftQueue(res.remaining);
          if (res.nextCriativoId) nextId = res.nextCriativoId;
        } catch {
          /* fallback local */
        }
      }
      if (nextId) {
        navigate({ to: config.editorPath, search: { criativoId: nextId, focus: "audio" } });
        toast.info("Próximo rascunho da fila");
        return;
      }
      navigate({ to: config.pipelinePath, search: { criativoId, ...pipelineSearchForMode(mode) } });
    } catch (e) {
  ...
  async function handleNextDraft() {
    ...
    if (nextId) {
      ...
      navigate({ to: config.pipelinePath, search: pipelineSearchForMode(mode) });
    }
  }
    ...
    <div className="h-[calc(100vh-3.5rem)] flex flex-col">
      ...
      {(showPostExport || criativo.export_status === "pronto") && exportPaths.length > 0 && (
        <PostExportBanner
          ...
          historicoSearch={pipelineSearchForMode(mode)}
        />
      )}
      <div className="px-4 lg:px-6 pt-4">
        ...
        <EditorOnboardingStrip
          hasAudio={Object.keys(audioPaths).length > 0 || !!audioUrl}
          hasMedia={!!backgroundUrl}
          hasScore={!!scoreData}
          exported={criativo.export_status === "pronto"}
          needsMedia={estilo === "clipes_texto" || estilo === "ugc_avatar"}
          onStepFocus={(focus) => {
            if (focus === "score") void handleAvaliar();
          }}
        />
        {landingAlignment && landingAlignment.score < 70 && criativo.export_status !== "pronto" && (
          <Card className="mt-3 border-warning/50 bg-warning/10 p-4 flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1.5 min-w-0">
              <p className="font-medium text-warning flex items-center gap-2 text-sm">
                <AlertTriangle className="size-4 shrink-0" />
                Congruência baixa com a landing ({landingAlignment.score}%)
              </p>
              <p className="text-xs text-muted-foreground">
                Hook ou CTA divergem da oferta na sua landing page.
                Ajuste o roteiro antes de exportar para proteger conversão.
              </p>
              <ul className="text-xs text-muted-foreground space-y-0.5">
                {landingAlignment.divergencias.slice(0, 3).map((d) => (
                  <li key={d}>• {d}</li>
                ))}
              </ul>
            </div>
            <Button size="sm" variant="outline" onClick={() => void handleLandingAlignment()} disabled={landingLoading}>
              {landingLoading ? <Loader2 className="size-3.5 animate-spin" /> : "Reanalisar"}
            </Button>
          </Card>
        )}
      ...
      <div className="flex items-center justify-between px-4 lg:px-6 py-3 border-b border-border/50 gap-2">
        ...
        <div className="hidden lg:flex gap-2 items-center">
          ...
          <Button id="editor-score" variant="outline" onClick={handleAvaliar}>Score</Button>
          <Dialog open={scoreOpen} onOpenChange={setScoreOpen}>
            <DialogTrigger asChild>
              <Button id="editor-export-panel" className="bg-gradient-primary border-0 shadow-glow" disabled={exporting}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <section
          className={`flex-1 flex flex-col items-center justify-center p-4 lg:p-6 bg-background/40 gap-4 ${
          ...
          {exportPaths.length > 0 && (() => {
            ...
            return mp4Url ? (
              <div className="w-full max-w-[270px]">
                <p className="text-xs text-muted-foreground mb-2 text-center">Preview do export</p>
                <video
                  src={mp4Url}
                  controls
                  className="w-full rounded-xl border border-border aspect-[9/16] bg-black object-contain"
                />
              </div>
            ) : null;
          })()}
      ...
      {planUsage && (
        <ExportLimitModal
          ...
          canUpgrade={currentOrg?.role === "owner"}
        />
      )}

      <AlertDialog open={exportCongruenceOpen} onOpenChange={setExportCongruenceOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2 text-warning">
              <AlertTriangle className="size-5" />
              Congruência baixa ({landingAlignment?.score ?? 0}%)
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-muted-foreground">
                <p>
                  O hook ou CTA deste criativo diverge da landing. Exportar assim pode reduzir conversão no Meta.
                </p>
                {landingAlignment?.divergencias.slice(0, 3).map((d) => (
                  <p key={d}>• {d}</p>
                ))}
                {landingAlignment?.sugestoes[0] && (
                  <p className="text-foreground">→ {landingAlignment.sugestoes[0]}</p>
                )}
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Revisar roteiro</AlertDialogCancel>
            <AlertDialogAction
              className="bg-warning text-warning-foreground hover:bg-warning/90"
              onClick={() => void runExportFlow()}
            >
